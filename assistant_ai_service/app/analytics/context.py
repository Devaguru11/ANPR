...
def location_descriptor(plan: AnalyticalPlan) -> str:
    scope = plan.entity_scope or {}
    loc = scope.get('location') or scope.get('display')
    if loc:
        return str(loc)
    cam = scope.get('camera_id') or plan.filters.get('location')
    return str(cam) if cam else ''
...
def entity_label(value: str, plan: AnalyticalPlan) -> str:
    if not value:
        return 'Unknown'
    v = str(value)
    if v.isupper() or (v.replace('_', '').isalnum() and '_' in v and (v.upper() == v)):
        return v
    if v in VIOLATION_LABELS:
        return VIOLATION_LABELS[v]
    return v.replace('_', ' ').title() if '_' in v else v
