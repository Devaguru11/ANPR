...
def state_from_plan(plan: dict[str, Any] | None) -> dict[str, Any]:
    ...
    location = entities.get('location') or entities.get('display')
    if not location and entities.get('camera_id'):
        location = str(entities.get('camera_id'))
    return {'metric': _metric_from_spec(agg, plan), 'location': location, 'entity_scope': copy_scope_dict(entities), 'filters': copy.deepcopy(plan.get('filters') or agg.get('filters') or {}), 'group_by': list(agg.get('group_by') or []), 'dimensions': list(agg.get('dimensions') or []), 'time_range': copy.deepcopy(plan.get('time_range') or {'preset': 'last_30_days'}), 'query_type': plan.get('query_type', 'aggregation'), 'intent': plan.get('intent', 'count'), 'limit': plan.get('limit', 10), 'compare_to': copy.deepcopy(plan.get('compare_to') or agg.get('compare_to') or {}), 'plate_suffix': plan.get('plate_suffix'), 'last_result_count': plan.get('last_result_count')}
...
def apply_scope_modifications(prev: dict[str, Any], question: str, entities: dict[str, Any], intent: str, filters: dict[str, Any], time_range: dict[str, Any] | None=None, *, inherit: bool=False, record_request: bool=False) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if entities:
        state['entity_scope'].update(copy_scope_dict(entities))
        if entities.get('location') or entities.get('display'):
            state['location'] = entities.get('location') or entities.get('display')
...
def update_scope_from_result(state: dict[str, Any], intent: str, columns: list[str], rows: list[tuple], camera_names: dict[str, str]) -> dict[str, Any]:
    ...
    if intent in ('top_n', 'camera_analysis') and data and ('camera_id' in data[0]):
        ...
        if cid:
            updated['entity_scope']['camera_id'] = cid
            updated['location'] = camera_names.get(str(cid), str(cid))
            updated['entity_scope']['location'] = updated['location']
            updated['entity_scope']['display'] = updated['location']
