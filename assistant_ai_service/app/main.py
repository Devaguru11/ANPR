...
from app.engines.legacy import router as legacy_router
...
app.include_router(legacy_router, prefix='/assistant')
...
async def health() -> dict[str, Any]:
    ...
    try:
        ...
        db_ok = False
    return {'status': 'ok' if llm_ok and redis_ok and db_ok else 'degraded', 'service': 'assistant_stable', 'engines': ['assistant'], 'llm': llm_ok, 'redis': redis_ok, 'database': db_ok, 'schema_tables': app.state.schema_tables, 'bind': f'{settings.ai_service_host}:{settings.ai_service_port}', 'analytics_redis_namespace': settings.analytics_redis_namespace}
