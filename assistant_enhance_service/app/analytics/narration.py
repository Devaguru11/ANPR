...
from app.analytics.context import VIOLATION_LABELS, dimension_label, entity_label, filter_descriptors, location_descriptor, metric_label, scope_phrase, time_context_phrase
...
def _narrate_ranking(plan: AnalyticalPlan, insight: InsightResult) -> str:
    ranking = _ranking_rows_from_insight(insight)
    (winner_name, winner_value) = _ranking_winner(insight, ranking)
    if winner_name is None or not isinstance(winner_value, (int, float)):
        return insight.summary or 'No ranking data available.'
    parts = [_ranking_lead_sentence(plan, winner_name, winner_value)]
    (runner_name, runner_value) = _ranking_runner(insight, ranking)
    if runner_name and isinstance(runner_value, (int, float)):
        parts.append(_ranking_runner_sentence(plan, runner_name, runner_value))
    return ' '.join(parts)

def _ranking_rows_from_insight(insight: InsightResult) -> list[dict[str, Any]]:
    ranking = insight.metrics.get('ranking') or []
    if ranking:
        return ranking
    if not insight.rankings:
        return []
    val_key = 'total'
    return [{'label': row.get('site_name') or row.get('camera_id') or row.get('label', 'unknown'), 'value': float(row.get(val_key, row.get('total', 0)) or 0)} for row in insight.rankings]

def _ranking_winner(insight: InsightResult, ranking: list[dict[str, Any]]) -> tuple[str | None, int | float | None]:
    winner_name = insight.metrics.get('winner_name') or insight.metrics.get('top_entity') or insight.metrics.get('largest_contributor')
    winner_value = insight.metrics.get('winner_value') if insight.metrics.get('winner_value') is not None else insight.metrics.get('largest_contributor_value')
    if winner_value is None:
        winner_value = insight.metrics.get('value')
    if ranking:
        winner_name = winner_name or ranking[0].get('label')
        if winner_value is None:
            winner_value = ranking[0].get('value')
    return (str(winner_name) if winner_name is not None else None, winner_value)

def _ranking_runner(insight: InsightResult, ranking: list[dict[str, Any]]) -> tuple[str | None, int | float | None]:
    ...
    if ranking and len(ranking) >= 2:
        ...
        if runner_value is None:
            runner_value = ranking[1].get('value')
    return (str(runner_name) if runner_name is not None else None, runner_value)

def _ranking_metric_phrase(plan: AnalyticalPlan) -> str:
    filters = filter_descriptors(plan)
    metric = metric_label(plan.metric)
    if filters:
        return f"{' '.join(filters)} {metric}"
    return metric

def _ranking_lead_sentence(plan: AnalyticalPlan, winner_name: str, winner_value: int | float) -> str:
    name = _ranking_display_name(winner_name, plan)
    val = int(winner_value) if float(winner_value).is_integer() else winner_value
    metric = metric_label(plan.metric)
    when = time_context_phrase(plan)
    when_suffix = f' {when}' if when else ''
    dim = (plan.dimensions or plan.group_by or [None])[0]
    if dim == 'hour':
        return f'{name} was the peak hour{when_suffix} with {val:,} {metric}.'
    if dim in ('day', 'date'):
        return f'{name} was the busiest day{when_suffix} with {val:,} {metric}.'
    if dim == 'month':
        return f'{name} was the busiest month{when_suffix} with {val:,} {metric}.'
    if dim == 'violation_type':
        return f'{name} was the most common violation type{when_suffix} with {val:,} {metric}.'
    if dim == 'vehicle_type':
        return f'{name} was the most common vehicle type{when_suffix} with {val:,} {metric}.'
    if dim == 'location':
        return f'{name} had the highest number of {_ranking_metric_phrase(plan)}{when_suffix} with {val:,} {metric}.'
    return f'{name} had the highest number of {_ranking_metric_phrase(plan)}{when_suffix} with {val:,} {metric}.'

def _ranking_runner_sentence(plan: AnalyticalPlan, runner_name: str, runner_value: int | float) -> str:
    name = _ranking_display_name(runner_name, plan)
    val = int(runner_value) if float(runner_value).is_integer() else runner_value
    return f'{name} followed with {val:,}.'

def _ranking_display_name(label: str, plan: AnalyticalPlan) -> str:
    dim = (plan.dimensions or plan.group_by or [None])[0]
    if dim == 'hour':
        return _format_period(label, plan)
    if dim in ('day', 'date', 'month'):
        return _format_period(label, plan)
    if dim == 'violation_type':
        return VIOLATION_LABELS.get(label, entity_label(label, plan))
    return user_facing_label(label, plan)
...
def _narrate_breakdown(plan: AnalyticalPlan, insight: InsightResult) -> str:
    ...
    top = ranking[0]
    top_name = user_facing_label(str(top.get('label', '')), plan)
    ...
    if len(ranking) >= 2:
        bottom = ranking[-1]
        bottom_name = user_facing_label(str(bottom.get('label', '')), plan)
...
def _narrate_growth(plan: AnalyticalPlan, insight: InsightResult) -> str:
    ...
    name = user_facing_label(str(entity), plan)
    dim = dimension_label(plan)
    scope = metric_label(plan.metric)
    ...
    if len(ranking) >= 2:
        second = ranking[1]
        second_name = user_facing_label(second.get('label', ''), plan)
    ...
    if decreases:
        ...
        worst_name = user_facing_label(worst.get('label', ''), plan)
...
def _narrate_records(plan: AnalyticalPlan, insight: InsightResult) -> str:
    ...
    for row in insight.records[:10]:
        ...
        site = row.get('site_name') or user_facing_label(row.get('camera_id', ''), plan)
