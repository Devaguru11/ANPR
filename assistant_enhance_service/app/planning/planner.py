...
from app.planning.dimension_promotion import promote_analysis_dimension
...
from app.planning.semantic_consistency import enforce_semantic_consistency
...
class AnalyticalPlanner:
    ...
    async def plan(self, question: str, conversation_context: str, previous_mem_plan: dict[str, Any] | None, entity_resolutions: list[dict[str, Any]], entity_scope: dict[str, Any], resolved_objective: ObjectiveResolution, resolved_dimension: DimensionResolution, resolved_temporal: TemporalResolution, retrieval_scope: str | None=None, business_resolution: BusinessSemanticResolution | None=None) -> tuple[AnalyticalPlan, dict[str, Any]]:
        ...
        self._normalize_plan(merged, resolved_dimension, question)
        if retrieval_scope:
        ...
        consistency = enforce_semantic_consistency(merged, resolved_dimension=resolved_dimension.dimension)
        from app.planning.semantic_normalization import apply_semantic_normalization
        semantic_norm = apply_semantic_normalization(merged)
        debug = {'inherit': inherit, 'modifications': modifications, 'proposed': proposed.to_dict(), 'previous': previous.to_dict() if previous else None, 'retrieval_scope': retrieval_scope, 'objective_resolution': resolved_objective.to_dict(), 'dimension_resolution': resolved_dimension.to_dict(), 'temporal_resolution': resolved_temporal.to_dict(), 'business_semantic_resolution': business_resolution.to_dict() if business_resolution else None, **promotion.to_dict(), **consistency.to_dict(), **semantic_norm.to_dict()}
        return (merged, debug)
    ...
    def _construct_plan(self, question: str, previous: AnalyticalPlan | None, entity_scope: dict[str, Any], resolved_objective: ObjectiveResolution, resolved_dimension: DimensionResolution, resolved_temporal: TemporalResolution, retrieval_scope: str | None=None, business_resolution: BusinessSemanticResolution | None=None) -> AnalyticalPlan:
        ...
        self._apply_resolved_semantics(plan, resolved_objective, resolved_dimension, resolved_temporal, question)
        apply_retrieval_scope(plan, retrieval_scope)
        return plan

    def _apply_resolved_semantics(self, plan: AnalyticalPlan, resolved_objective: ObjectiveResolution, resolved_dimension: DimensionResolution, resolved_temporal: TemporalResolution, question: str='') -> None:
        plan.user_objective = resolved_objective.objective
        plan.time_range = dict(resolved_temporal.time_range)
        self._apply_objective(plan, question)
    ...
    def _apply_transformable_semantics(self, plan: AnalyticalPlan, resolved_objective: ObjectiveResolution, resolved_dimension: DimensionResolution) -> None:
        plan.user_objective = resolved_objective.objective
    ...
    def _apply_objective(self, plan: AnalyticalPlan, question: str='') -> None:
        obj = plan.user_objective
        if obj not in objective_names():
            obj = default_objective()
            plan.user_objective = obj
        explicit_time = question_has_explicit_time(question)
        ...
        if obj == 'metric_summary':
            ...
            if not explicit_time and plan.time_range.get('preset') in (None, 'last_30_days'):
                plan.time_range = {'preset': 'this_month'}
            if not explicit_time:
                plan.compare_to = {'preset': 'last_month'}
        else:
    ...
    def _normalize_plan(self, plan: AnalyticalPlan, resolved_dimension: DimensionResolution, question: str='') -> None:
        (plan.filters, plan.time_range) = relocate_time_filters(plan.filters, plan.time_range)
        if plan.query_mode == 'comparison' and (not question_has_explicit_time(question)):
            plan.time_range = {'preset': 'this_month'}
            plan.compare_to = {'preset': 'last_month'}
            plan.dimensions = []
            plan.group_by = []
