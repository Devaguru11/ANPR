...
from app.planning.semantic_model_registry import registry_prompt as semantic_model_prompt
...
from app.planning.reference_resolver import ReferenceResolution, apply_resolved_reference
...
class UnifiedSemanticResolver(SemanticResolver):
    ...
    async def resolve_unified(self, question: str, conversation_context: str, previous_mem_plan: dict[str, Any] | None, mem_time_range: dict[str, Any] | None=None, reference_resolution: ReferenceResolution | None=None) -> UnifiedResolveResult:
        timing = ResolutionTiming()
        ...
        ref_time_range = None
        if reference_resolution and reference_resolution.has_analytical_reference:
            (semantic.entities, ref_scope, ref_tr) = apply_resolved_reference(semantic.entities, prev_scope, semantic.time_range, reference_resolution, question=question)
            ref_time_range = reference_resolution.resolved_reference.get('time_range') if reference_resolution.resolved_reference.get('time_range') else None
            if ref_tr != semantic.time_range:
                semantic.time_range = ref_tr
            prev_scope = {**prev_scope, **ref_scope}
            semantic.reasoning += f'; reference: {reference_resolution.reasoning[:120]}'
            semantic.resolution_tier = 'reference_merge'
        semantic = self._normalize_time(semantic, question, reporting, partial_date, active_tr, inherit, reference_time_range=ref_time_range if isinstance(ref_time_range, dict) else {'preset': ref_time_range} if isinstance(ref_time_range, str) else None)
    ...
    def _time_for_cache_hit(self, question: str, reporting: dict[str, Any], partial_date: dict[str, Any] | None, active_tr: dict[str, Any], inherit: bool) -> dict[str, Any]:
        ...
        result = apply_temporal_precedence(question, active=active_tr, partial_date=partial_date, inherit=inherit)
