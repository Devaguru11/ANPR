    ...
    def resolve_location(self, text: str) -> tuple[dict | None, float, list]:
        ...
        for (cid, name) in self.cache.camera_names.items():
            if name.lower() == phrase_l:
                return ({'camera_id': cid, 'location': name, 'display': name, 'display_name': name}, 1.0, [{'type': 'camera', 'value': cid, 'display': name, 'confidence': 1.0, 'source': 'exact_name'}])
        ...
        if len(matched) == 1:
            best = {'camera_id': matched[0], 'location': names[0], 'display': names[0], 'display_name': names[0]}
        else:
            display = ', '.join(names[:3])
            best = {'camera_id': matched, 'location': phrase, 'display': display, 'display_name': display}
